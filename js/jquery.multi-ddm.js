	...
	$.fn.dropDownMenu = function( options ) {
		return this.each(function(){
			...
			$main.delegate( 'li', 'mouseenter.multi-ddm', function(){
				...
				// Recuperar largura da tela
				var win_w = $(window).width();

				// Open new menu and remove any lingering hover elements
				self.children('a').addClass( $.data( el.parentNode, 'multi-ddm-classname' ) ).siblings('ul').show().children('li').each( clearSiblings );
				
				// Se não for o primeiro nivel de menu
				if (!self.hasClass('n0'))
				{
					// Recuperar largura do menu
					var men_w = self.children('a').siblings('ul').width();
					
					// Verifica se deve por do lado direito ou esquerdo
					if ((men_w + (self.width()-1) + self.offset().left) <= win_w)
						self.children('a').siblings('ul').css("left", ((self.width()-1) + "px"));
					else
						self.children('a').siblings('ul').css("left", (((men_w-1)*-1) + "px"));
				}
			})
